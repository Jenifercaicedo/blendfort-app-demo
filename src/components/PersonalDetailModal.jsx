...
const PersonalDetailModal = ({ empleado, onClose, onEdit, onDelete }) => {
  ...
  const tipo = empleado.tipo || "CAMPO";
  const rol = empleado.rol || "OPERARIO";
  const esOficina = tipo === "OFICINA";

  const salarioMensual = empleado.salarioMensual ?? "";
  const valorDia = empleado.valorDia ?? "";
  const valorHoraExtra = empleado.valorHoraExtra ?? 0;
      ...
      <div className="bg-white w-full max-w-md rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-black/5">
        ...
        <div className="bg-black text-white relative px-8 pt-10 pb-8">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10">
              <span className="text-[7px] font-black uppercase tracking-[0.25em] text-white/50">ROL</span>
              <span className="text-[8px] font-black uppercase tracking-widest">{rol}</span>
            </span>

            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10">
              <span className="text-[7px] font-black uppercase tracking-[0.25em] text-white/50">TIPO</span>
              <span className="text-[8px] font-black uppercase tracking-widest">{tipo}</span>
            </span>

            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/10">
              <span className="text-[7px] font-black uppercase tracking-[0.25em] text-white/50">PROY</span>
              <span className="text-[8px] font-black uppercase tracking-widest">
                {empleado.proyecto || "SIN ASIGNAR"}
              </span>
            </span>
          </div>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="bg-blendfort-fondo rounded-[2.5rem] border border-black/5 p-6">
            ...
            <div className="space-y-3 text-[10px]">
              <div className="flex justify-between items-center">
                <span className="text-[8px] font-black uppercase opacity-20 tracking-widest">Fecha contratación</span>
                <span className="font-black uppercase tracking-tight text-black">
                  {formatFecha(empleado.fechaContratacion)}
                </span>
              </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-black/5 p-6 shadow-sm">
            ...
            <div className="space-y-3 text-[10px]">
              <div className="flex justify-between items-center">
                <span className="text-[8px] font-black uppercase opacity-20 tracking-widest">Salario mensual</span>
                <span className="font-black uppercase tracking-tight text-black">
              ...
              <div className="flex justify-between items-center">
                <span className="text-[8px] font-black uppercase opacity-20 tracking-widest">Valor día</span>
                <span className="font-black uppercase tracking-tight text-black">
                  {valorDia !== "" && valorDia !== null && valorDia !== undefined ? money(valorDia) : "—"}
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-[8px] font-black uppercase opacity-20 tracking-widest">Hora extra</span>
                <span className="font-black uppercase tracking-tight text-blendfort-naranja">
                  {money(valorHoraExtra)}
                </span>
              </div>

              {/* Línea de resumen (lo relevante) */}
              <div className="pt-4 mt-2 border-t border-black/5 flex items-center justify-between">
                ...
                <span className="text-[11px] font-black uppercase tracking-tight text-black">
                  {esOficina ? `${money(salarioMensual || 0)} / MES` : `${money(valorDia || 0)} / DÍA`}
                </span>
              </div>
            </div>
          </div>
