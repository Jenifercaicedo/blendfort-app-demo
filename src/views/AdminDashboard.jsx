...
const Toast = ({ mensaje, tipo = "exito", onClose }) => {
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className={`${estilos[tipo]} px-7 py-3 rounded-full shadow-xl flex items-center gap-3 border border-white/10`}>
...
const AdminDashboard = () => {
  ...
  <div className="max-w-7xl mx-auto flex justify-between items-center mb-8 md:mb-12 px-2">
    <button
      type="button"
      onClick={() => {
    ...
    >
      <span className="text-[10px] font-semibold uppercase tracking-[0.12em] text-black/70 group-hover:text-black transition-colors">
        Cerrar Sesión
      </span>
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 gap-5 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl lg:text-4xl font-black tracking-tight uppercase leading-none">
                Reporte Maestro
              </h1>
            </div>
            <p className="text-[9px] font-black opacity-30 uppercase tracking-[0.28em] ml-11">
          ...
          <div className="flex flex-row items-center gap-3 w-full lg:w-auto">
            <button
              ...
              className="group flex-1 lg:flex-none bg-white border border-black/10 px-6 py-3.5 rounded-[1.2rem] transition-all hover:border-black hover:shadow-lg active:scale-95 flex items-center gap-3"
              type="button"
            >
              <div className="w-8 h-8 rounded-xl bg-black text-white flex items-center justify-center text-base font-light group-hover:bg-blendfort-naranja transition-colors">
                +
              </div>

              <div className="text-left">
                <span className="block text-[8px] font-black opacity-30 uppercase tracking-[0.08em] leading-none">
                  Master Setup
                </span>
                <span className="text-[13px] md:text-[14px] font-black uppercase tracking-[0.01em] leading-none mt-1 block">
            ...
            <button
              ...
              className="group flex-1 lg:flex-none relative bg-blendfort-naranja px-6 py-3.5 rounded-[1.2rem] transition-all duration-500 hover:bg-black hover:shadow-lg active:scale-95 flex items-center gap-3"
              type="button"
            >
              <div className="w-8 h-8 rounded-xl bg-white/20 text-white flex items-center justify-center text-base font-light group-hover:bg-blendfort-naranja transition-all">
                +
              </div>

              <div className="text-left">
                <span className="block text-[8px] font-black text-white/55 uppercase tracking-[0.08em] leading-none">
                  Quick Record
                </span>
                <span className="text-[13px] md:text-[14px] font-black text-white uppercase tracking-[0.01em] leading-none mt-1 block">
      ...
      {!seccionActiva && (
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mb-10 animate-in fade-in zoom-in-95 duration-500">
          {[
            ...
            { id: "cajaChica", num: "05", title: "Caja Chica", desc: "Fondos por Proyecto" },
          ].map((card) => (
            <button
              key={card.id}
              onClick={() => setSeccionActiva(card.id)}
              className="group relative p-6 md:p-7 rounded-[1.5rem] text-left transition-all duration-500 border bg-white border-black/5 hover:border-black shadow-sm hover:shadow-md"
              type="button"
            >
              <div className="relative z-10 space-y-3">
                <div className="w-8 h-[2px] bg-black/10 group-hover:bg-blendfort-naranja transition-colors"></div>
                <div>
                  <h3 className="text-base md:text-lg font-black uppercase tracking-tight leading-tight">{card.title}</h3>
                  <p className="text-[8px] font-black opacity-40 uppercase tracking-[0.16em] mt-1">
                    {card.desc}
                  </p>
                </div>
              </div>
              <div className="absolute -bottom-3 -right-1 text-7xl md:text-8xl font-black text-black opacity-[0.02] group-hover:opacity-[0.05] transition-opacity">
                {card.num}
              </div>
            </button>
          ))}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {seccionActiva === "manoDeObra" && (
          <ManoObraCard onBack={() => setSeccionActiva(null)} />
        )}
        ...
        {seccionActiva === "gestionPersonal" && (
          <GestionPersonal onBack={() => setSeccionActiva(null)} />
        )}

        {seccionActiva === "cajaChica" && (
          <CajaChicaView onBack={() => setSeccionActiva(null)} />
        )}
      </div>
      ...
      <footer className="mt-10 text-center opacity-20 text-[8px] font-bold uppercase tracking-[0.32em]">
